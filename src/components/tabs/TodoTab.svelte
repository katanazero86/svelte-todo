<script lang="ts">
    ...
    const ITEMS: readonly string[] = ['ALL', 'DONE'];
    const activeColor:string = '#9a78d6';
    const noActiveColor:string = '#94a3b8';
    const noActiveClass:string = 'bg-gray-100 hover:bg-violet-50 border-slate-200';
...
<div class="flex flex-row items-center justify-between cursor-pointer divide-x divide-slate-200">
    <div class={`basis-1/2 flex justify-center hover:bg-violet-50 py-3 border-b border-transparent ${activeTab !== ITEMS[0] && noActiveClass}`}
         on:click={handleTabClick(ITEMS[0])}>
        <svg xmlns="http://www.w3.org/2000/svg" height="34" viewBox="0 0 20 20"
             width="34" fill={activeTab === ITEMS[0] ? activeColor : noActiveColor}>
    ...
    <div class={`basis-1/2 flex justify-center hover:bg-violet-50 py-3 border-b border-transparent ${activeTab !== ITEMS[1] && noActiveClass}`}
         on:click={handleTabClick(ITEMS[1])}>
        <svg xmlns="http://www.w3.org/2000/svg" height="34" viewBox="0 0 24 24" width="34" fill={activeTab === ITEMS[1] ? activeColor : noActiveColor}>
